...
//
// Original source: github.com/micro/go-micro/v3/store/minio/minio.go
...
// NewBlobStore returns an initialized minio blob store
func NewBlobStore(opts ...Option) (store.BlobStore, error) {
	...
	if options.TLSConfig != nil {
		...
		if err != nil {
			return nil, errors.Wrap(err, "Error setting up minio blob store transport")
		}
	...
	if err != nil {
		return nil, errors.Wrap(err, "Error connecting to minio blob store")
	}
...
func (s *s3) Read(key string, opts ...store.BlobOption) (io.Reader, error) {
	...
	// make the key safe for use with minio
